...
Version `1.1.8` includes an opt-in **Gym Challenge** flow with a direct Gen 1 offer trigger. In Red, Blue, and Yellow, the final native Oak parcel/Pokédex cutscene queues the offer after the player chooses a starter, defeats the Oak’s Lab rival, returns Oak’s Parcel, receives the Pokédex, and the rival departs. It does not require leaving and re-entering the lab. Crystal 251 does not change this Oak’s Lab sequence; its optional integration only provides merged roster data. In Gold, the offer appears after the player receives a starter, returns with the Mystery Egg, and hands it to Elm. Accepting shows the first gym, saved preset, and enabled rules before safely updating the already received starter, healing the party, and routing to the first physical gym.
