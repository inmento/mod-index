...
Version `1.1.7` includes an opt-in **Gym Challenge** flow with a corrected Gen 1 offer trigger. In Red, Blue, and Yellow, the offer appears after the player chooses a starter, defeats the Oak’s Lab rival, returns Oak’s Parcel, receives the Pokédex, and the full Oak’s Lab cutscene finishes. Players who already completed that sequence can update and re-enter Oak’s Lab to receive the one-time offer. In Gold, it appears after the player receives a starter, returns with the Mystery Egg, and hands it to Elm. Accepting shows the first gym, saved preset, and enabled rules before safely updating the already received starter, healing the party, and routing to the first physical gym.
