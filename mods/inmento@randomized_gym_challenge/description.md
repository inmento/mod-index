...
Each building keeps its original badge, reward, and progression flow. With leader randomization enabled, the visiting leader supplies the battle presentation while the physical gym still owns completion rewards. Gold held-item generation excludes key items, HMs, non-tossable items, and no-effect held items.

> This is a **pre-release experimental build**. Test it on a separate save and use the repository’s testing checklist when reporting issues.
